...
function followBtnClickHandler(id) {
    const follow = ['follow', 'Читать', 'hover-blue'];
    const unfollow = ['unfollow', 'Перестать читать', 'hover-red'];
    let btn = $(`#follow-btn-${id}`)[0]
    console.log(id)
    console.log($(`#follow-btn-${id}`))
    // if (btn.dataset.action === follow[0]){
    //     followFetch(btn.dataset.followurl, unfollow[1], unfollow[2], follow[2], unfollow[0])
    // }
    // else{
    //     followFetch(btn.dataset.unfollowurl, follow[1], follow[2], unfollow[2], follow[0])
    // }
}
